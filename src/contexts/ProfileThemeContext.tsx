...
type ProfileTheme =
  | typeof STUDENT_THEME
  | typeof INSTRUCTOR_THEME
  | typeof ADMIN_THEME;
 ...
 */
export const ProfileThemeProvider: React.FC<ProfileThemeProviderProps> = ({
  children,
}) => {
  ...
  const userType = (userProfile?.userType || "student") as
    | "student"
    | "instructor"
    | "admin";
...
export const useProfileTheme = (): ProfileThemeContextType => {
  ...
  if (!context) {
    // Fallback seguro para evitar quebrar o app
    console.warn(
      "useProfileTheme used outside ProfileThemeProvider, using fallback theme",
    );
    return {
      theme: STUDENT_THEME,
      userType: "student",
