...
class PaymentService {
  ...
  // Confirmar pagamento
  async confirmPayment(paymentId, confirmationData, academiaId) {
    try {
      if (!academiaId) {
        throw new Error('academiaId é obrigatório para confirmar pagamento');
      }
      ...
      await firestoreService.updateDocument(`gyms/${academiaId}/payments`, paymentId, updateData);
  ...
  async checkOverduePayments(academiaId) {
    try {
      if (!academiaId) {
        throw new Error('academiaId é obrigatório para verificar pagamentos vencidos');
      }

      const today = new Date();
      today.setHours(0, 0, 0, 0);

      // Buscar pagamentos vencidos da academia específica
      const overduePayments = await firestoreService.getDocuments(
        `gyms/${academiaId}/payments`,
        [
