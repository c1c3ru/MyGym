...
export const academyFirestoreService = {
  ...
  getWhere: async (collectionName, field, operator, value, academiaId = null) => {
    try {
      // Validar parâmetros para evitar erros do Firestore
      if (!field || !operator) {
        console.warn('❌ Campo ou operador inválido na query:', { field, operator });
        return [];
      }

      if (value === null || value === undefined) {
        console.warn('❌ Valor null/undefined na query, retornando array vazio:', { field, operator, value });
        return [];
      }
