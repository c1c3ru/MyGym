  ...
  getDocuments: async (collectionName, filters = [], orderByConfig = null, limitCount = null) => {
    try {
      ...
      filters.forEach(filter => {
        ...
        // Validar valor do filtro
        if (filter.value === null || filter.value === undefined) {
          console.warn('Null/undefined filter value skipped:', filter);
          return;
        }
...
export const studentService = {
  getStudentsByClass: async (classId) => {
    ...
    // Nota: Esta função pode precisar ser atualizada para usar academyFirestoreService
    const usersInClass = await firestoreService.getWhere('users', 'classIds', 'array-contains', classId);
    return usersInClass; // Todos os usuários em uma turma são considerados alunos
  },
  ...
  getStudentsByInstructor: async (instructorId) => {
    ...
    return usersInClasses; // Todos os usuários em turmas são considerados alunos
  },
