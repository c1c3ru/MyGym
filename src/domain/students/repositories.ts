// Repository interfaces for students domain

import { Student, StudentProfile, CreateStudentData, UpdateStudentData, StudentStats } from './entities';
...
export interface StudentGraduationsRepository {
  ...
  addGraduation(academiaId: string, studentId: string, graduation: Omit<import('./entities').Graduation, 'id'>): Promise<import('./entities').Graduation>;
  getGraduations(academiaId: string, studentId: string): Promise<import('./entities').Graduation[]>;
  updateGraduation(academiaId: string, studentId: string, graduationId: string, data: Partial<import('./entities').Graduation>): Promise<import('./entities').Graduation>;
  deleteGraduation(academiaId: string, studentId: string, graduationId: string): Promise<void>;
  
  // Graduation queries
  getGraduationsByLevel(academiaId: string, level: string): Promise<import('./entities').Graduation[]>;
  getGraduationsByModality(academiaId: string, modality: string): Promise<import('./entities').Graduation[]>;
  getGraduationsByInstructor(academiaId: string, instructorId: string): Promise<import('./entities').Graduation[]>;
  getGraduationsByDateRange(academiaId: string, startDate: Date, endDate: Date): Promise<import('./entities').Graduation[]>;
}
