...
const StudentProfileScreen: React.FC<StudentProfileScreenProps> = ({
  ...
  const loadStudentDetails = async () => {
    try {
      ...
      if (!details) {
        details = (await academyFirestoreService.getById(
          "students",
          studentId,
          academiaId,
        )) as StudentData;
      ...
      const allClasses = (await academyFirestoreService.getAll(
        "classes",
        academiaId,
      )) as ClassData[];
      const userClasses = allClasses.filter((cls) =>
        details?.classIds?.includes(cls.id),
      );
      setStudentClasses(userClasses);

      // Payments
      try {
        const userPayments = (await academyFirestoreService.getWhere(
          "payments",
          "studentId",
          "==",
          studentId,
      ...
      // Graduations
      try {
        const userGraduations = (await academyFirestoreService.getWhere(
          "graduations",
          "studentId",
          "==",
          studentId,
                  ...
                  <View style={styles.finBox}>
                    <Text style={dynamicStyles.finAmount}>
                      {formatCurrency(
                        payments.reduce(
                          (sum, p) =>
                            p.status === "pending" ? sum + p.amount : sum,
                          0,
                        ),
                      )}
                    </Text>
...
const styles = StyleSheet.create({
  ...
  beltChip: {
    height: 28,
    backgroundColor: hexToRgba(COLORS.warning[500], 0.1),
  },
