...
const ProfileScreen = ({ navigation }) => {
  ...
  const loadStudentData = async () => {
    ...
    try {
      ...
      if (!userId || !academiaId) {
        console.warn('Valores inválidos para carregar dados do estudante');
        return;
      }
      ...
      const evaluations = await firestoreService.getDocuments(
        `gyms/${academiaId}/physicalEvaluations`,
        [{ field: 'userId', operator: '==', value: userId }]
      );
  ...
  const loadCurrentPayment = async () => {
    ...
    try {
      ...
      if (!userId || !academiaId) {
        console.warn('Valores inválidos para carregar pagamento atual');
        return;
      }
