...
const PhysicalEvaluationHistoryScreen = ({
  navigation,
}: PhysicalEvaluationHistoryScreenProps) => {
  ...
  const getChartData = () => {
    ...
    const labels = sortedEvaluations.map((evaluation) => {
      const date = evaluation.date.toDate
        ? evaluation.date.toDate()
    ...
    switch (selectedMetric) {
      ...
      case "bodyFat":
        data = sortedEvaluations
          .filter((evaluation) => evaluation.bodyFat)
          .map((evaluation) => evaluation.bodyFat);
        suffix = "%";
        break;
      case "muscleMass":
        data = sortedEvaluations
          .filter((evaluation) => evaluation.muscleMass)
          .map((evaluation) => evaluation.muscleMass);
        suffix = " kg";
        break;
      default:
        data = sortedEvaluations.map((evaluation) => evaluation.weight);
      ...
      datasets: [
        {
          data: data.slice(-10),
          strokeWidth: 3,
          color: (opacity = 1) => hexToRgba(COLORS.info[500], opacity),
        },
      ],
      suffix,
    };
  ...
  const metrics = [
    {
      key: "weight",
      label: getString("weightKg").replace(" (kg)", ""),
      icon: "scale-outline",
    },
    { key: "imc", label: "IMC", icon: "calculator-outline" },
    {
      key: "bodyFat",
      label: getString("bodyFatPercent").replace(" (%)", ""),
      icon: "body-outline",
    },
    {
      key: "muscleMass",
      label: getString("muscleMassKg").replace(" (kg)", ""),
              ...
              <View style={styles.statsGrid}>
                <Surface style={styles.statItem}>
                  <Text style={styles.statValue}>
                    {latestEvaluation.weight} kg
                  </Text>
                  <Text style={styles.statLabel}>
                    {getString("weightKg").replace(" (kg)", "")}
                  </Text>
                </Surface>
                ...
                <Surface style={styles.statItem}>
                  <Text style={styles.statValue}>
                    {latestEvaluation.height} cm
                  </Text>
                  <Text style={styles.statLabel}>
                    {getString("heightCm").replace(" (cm)", "")}
                  </Text>
                </Surface>

                <Surface style={styles.statItem}>
                  <Text style={styles.statValue}>
                    {latestEvaluation.age}{" "}
                    {getString("ageYears").replace(" (anos)", "")}
                  </Text>
                  <Text style={styles.statLabel}>
                    {getString("ageYears").replace(" (anos)", "")}
                  </Text>
            ...
            <View style={styles.glassCard}>
              <View style={styles.cardHeader}>
                ...
                <Text style={styles.subtitle}>
                  {getString("vsPreviousEvaluation")}
                </Text>
              </View>
              ...
              <View style={styles.progressGrid}>
                ...
                {progress.bodyFat !== null && (
                  <View style={styles.progressItem}>
                    <Text style={styles.progressLabel}>
                      {getString("bodyFatPercent").replace(" (%)", "")}
                    </Text>
                    <Text
                      style={[
                        styles.progressValue,
                ...
                {progress.muscleMass !== null && (
                  <View style={styles.progressItem}>
                    <Text style={styles.progressLabel}>
                      {getString("muscleMassKg").replace(" (kg)", "")}
                    </Text>
                    <Text
                      style={[
                        styles.progressValue,
                    ...
                    >
                      {progress.muscleMass > 0 ? "+" : ""}
                      {progress.muscleMass.toFixed(1)} kg
                    </Text>
                  </View>
                )}
              </View>
            ...
            <View style={styles.glassCard}>
              ...
              <LineChart
                ...
                chartConfig={{
                  ...
                  color: (opacity = 1) => hexToRgba(COLORS.info[500], opacity),
                  labelColor: (opacity = 1) => hexToRgba(COLORS.black, opacity),
                  style: {
            ...
            {evaluations.length === 0 ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyText}>
                  {getString("noEvaluationsFound")}
                </Text>
                <Text style={styles.emptySubtext}>
                  {getString("startEvaluationPrompt")}
                </Text>
              </View>
            ) : (
