...
const CheckInModalContent: React.FC<CheckInModalProps> = ({ onClose, onSuccess }) => {
    ...
    return (
        <EnhancedErrorBoundary errorContext={{ screen: 'CheckInModal' }}>
            <View style={styles.container}>
                <View style={styles.header}>
                    <Text style={styles.title}>{getString('checkIn')}</Text>
                    <IconButton icon="close" onPress={onClose} iconColor={colors.onSurface} />
                </View>
...
const createStyles = (colors: any) => StyleSheet.create({
    container: { flex: 1, backgroundColor: colors.background, borderRadius: BORDER_RADIUS.lg, overflow: 'hidden' },
    loadingContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', minHeight: 200 },
    scrollView: { flex: 1, maxHeight: Platform.OS === 'web' ? '80vh' as any : undefined },
    ...
    subtitle: { fontSize: FONT_SIZE.md, color: colors.onSurfaceVariant, marginBottom: SPACING.md },
